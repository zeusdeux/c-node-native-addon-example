...
build: clean fetch-node-headers
	$(CC) $(CFLAGS) $(SOURCES) -o $(BUILD_DIR)/addon.node

fetch-node-headers:
	mkdir -p $(BUILD_DIR)
	npm run prep-node-headers # we invoke node-gyp via npm so that node-gyp that ships with npm is used and because I can't be arsed to find out where in the npm install dir node-gyp is
